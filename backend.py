...
def process_image(image, flip, rotate, bw, resize, flip_type, rotate_degree, resize_percentage):
    # Debugging prints
    ...
    download_path = "processed_image.png"
    image.save(download_path, format="PNG")

    # If resizing is requested, return HTML to resize the image using CSS
    if resize:
        html = f"""
        <div style="display: flex; justify-content: center;">
            <img src="data:image/png;base64,{img_str}" style="width: {resize_percentage}%; height: auto;">
        </div>
        """
        return html, download_path

    # If no resizing, return the image as base64 string for display
    return f'<img src="data:image/png;base64,{img_str}" style="width: auto; height: auto;">', download_path
